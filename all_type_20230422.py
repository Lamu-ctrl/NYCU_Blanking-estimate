...
def ch_tool(degree, tool, square):  #選擇要用的刀具, square：是否要選用正方刀(0/1)
...
def line(line_start_x, line_start_y, distance, line_deg, tool, offset_index, line_serial_num, sub_serial_num):
    ...
    if (tool[tool_num,2] == -1):        #圓刀nib
        ...
        x_offset = r * math.sin(line_deg) #+ (line_nib_overlap/2) * math.cos(line_deg)
        y_offset = orient * r * math.cos(line_deg) #- (line_nib_overlap/2) * math.sin(line_deg)
        
        hit_type = 40
        line_track= 0   #nib的直線運行不列入記錄
        
        line_xy.append([line_start_x + x_offset, line_start_y + y_offset, tool_num, hit_type, line_serial_num, sub_serial_num])
        ...
        while (a>0) :
            ...
            line_xy.append([line_x, line_y, tool_num, hit_type, line_serial_num, sub_serial_num])
...
def circle(r, start_deg, cen_x, cen_y, parentID, tool, punch_type, cir_serial_num):
    ...
    if (parentID != 0):               #除料範圍為圓內
        ...
        if r == tool[tool_num, 0]:       # 刀具等於洞的大小時
            single_hit_cir += 1
            cir_xy.append([cen_x, cen_y, tool_num, 17, cir_serial_num]) # punch_type先用17，模擬方便
            ...
            while angle < (2*np.pi):
                ...
                nib_hit_cir += 1
                cir_xy.append([x_point, y_point, tool_num, 39, cir_serial_num])
                angle += feeding_a
        ...
        while angle < (2*np.pi):
            ...
            cir_xy.append([x_point, y_point, tool_num, 39, cir_serial_num, 1])
            nib_hit_cir += 1
            angle += feeding_a
...
def arc(cen_x, cen_y, r, s_deg, working_deg, orientation, parentID, station, line_serial_num, sub_serial_num):
    ...
    while angle < working_deg:
        ...
        arc_xy.append([x_point, y_point, tool_num, 39, line_serial_num, sub_serial_num])     ### arc的punch type 只會是39
        nib_hit_arc += 1
        angle += feeding_a
...
def xy_data_rotate(befor_rota_xy, sp2, degree):
    ...
    for k1 in range(all_hole_hit):  #將每一點都乘上旋轉矩陣，再加上第一個洞的原點
        ...
        af_rota_xy.append([x_abs, y_abs, tool_num, punch_type, rect_serial_num, rect_subserial_num])
        #將點位記錄在 xy_data 內
...
def rect_hole(L, H, sp1, punch_type, station, roDeg, rect_serial_num, rect_subserial_num):      #做法為：以單一矩形洞做全孔加工，剩下的陣列洞另外處理
    ...
    if (L==L_tool) & (H==w_tool):
        ...
        hole_xy.append([L_tool/2, H-w_tool/2, tool_num, punch_type, rect_serial_num, rect_subserial_num])
        
    else:
        hole_xy.append([L_tool, H-w_tool/2, tool_num, punch_type, rect_serial_num, rect_subserial_num])   #第一點位置，記錄點的型式為[x, y , 刀編號]
        ...
        while True :     #以橫向加工到底後垂直移動一些，再繼續橫向加工(b>0)
            ...
            while a>0 :
                ...
                hole_xy.append([x_hole, y_hole, tool_num, punch_type, rect_serial_num, rect_subserial_num])  #將點位記錄在hole_xy內
            ...
            hole_xy.append([x_hole, y_hole, tool_num, punch_type, rect_serial_num, rect_subserial_num])
    ...
    for k1 in range(all_hole_hit):  #將每一點都乘上旋轉矩陣，再加上第一個洞的原點
        ...
        hole_xy_data.append([x_abs, y_abs, tool_num, punch_type, rect_serial_num, rect_subserial_num])    #將點位記錄在 xy_data 內
...
def rect_outside_edge(L, H, sp1, station, roDeg, rect_serial_num):  #做矩形外圍的加工 ## roDeg是弳度制
    ...
    rect_punch_type = 34
    #橫向
    tool_num = ch_tool(roDeg, station, 0) #取刀具的資料
    L_tool = station[tool_num, 0]
    w_tool = station[tool_num, 1]
    ...
    rect_xy.append([x_start, y_start, tool_num, rect_punch_type, rect_serial_num, 1]) #外邊緣開始工作的第一點
    ...
    for k1 in range(2):
        ...
        while a>0:
            ...
            rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, k1+1])
        ...
        rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, 2])
    ...
    rect_xy.append([x_start, y_start, tool_num, rect_punch_type, rect_serial_num, 3]) #外邊緣開始工作的第一點
    ...
    for k1 in range(2):
        ...
        while b>0:
            ...
            rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, k1+3])
        ...
        rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, 4])
    ...
    for ix in range(len(xy_data)):
        ...
        if (ix != (len(xy_data) - 1)):    # 當不是最後一點時，進入迴圈
            ...
            if (one_xy_data[2] != next_xy_data[2]) & (any(pre_pt)): ## 換刀時，前一點必為原點
                pre_pt = [0, 0, 0, 0, 0, 0]
                turret_tool_change += 10
                #假設turret可以放36把刀，預設加工前將每把順序排好，因此每次換刀turret轉10度
            else:
                pre_pt = one_xy_data
        ## END ##for ix in range(len(xy_data))
            
    feature_data.append([" ", horizontal_line_hit, vertical_line_hit, single_hit, nibbling_hit, shear_hit,\
...
if __name__ == '__main__':
    ...
    for k1 in range(len(rect_var)):
        ...
        if parentID==0:     ## rect_outside_edge
            rect_xy, h_t_out, v_t_out, hpm_out, vpm_out, h_lhit_out,\
                     v_lhit_out = rect_outside_edge(L, H, sp1, station, roDeg, rect_serial_num)
            for j1 in range(len(rect_xy)):
                j2 = rect_xy[j1]
                all_rect.append(j2)

        else:       ## hole
            hole_xy_data, h_track, v_track\
                          = rect_hole(L, H, sp1, punch_type, station, degree, rect_serial_num, rect_subserial_num)
    ...
    for k2 in range(len(line_var)):
        ...
        line_xy, line_h_track, line_v_track, line_h_shearbwn, line_v_shearbwn, h_line_hit,\
               ...
               = line(line_start_x, line_start_y, distance, line_deg, station, offset_index,\
                      line_serial_num, line_sub_serial_num)
    ...
    for k3 in range(len(circle_var)):
        ...
        cir_xy, h_track_cir, v_track_cir, single_hit_cir, nib_hit_cir, cir_hpm, cir_vpm\
                = circle(r, start_deg, cen_x, cen_y, parentID, station, punch_type, cir_serial_num)
