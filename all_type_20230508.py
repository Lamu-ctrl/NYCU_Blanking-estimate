...
def readfile(path):
    ...
    while True:
        ...
        s2 = s2.strip()
        s3 = s2.replace(',',' ').split()    #remove s2's space, comma, and split it
        if (len(s3) <= 0):
            break
    ...
    while k < len(wholedata):
        ...
        #######################
            ...
            if (onerow_data[6]==18):            ##判斷是不是grid
                ...
                for ix in range(row_arr):
                    for iy in range(col_arr):
                        ...
                        rect_var.append([L, H, rect_grid_xy, parentID, onerow_data[6], roDeg, onerow_data[0], (ix + iy + 1)])
            ...
            #######################
                ...
                rect_var.append([L, H, rect_start_p, parentID, onerow_data[6], roDeg, onerow_data[0], 0]) 
                k = k+2         #將下一行也跳過
        ...
        ## alone a line-circle
        #######################
        elif (onerow_data[7]==17) & (onerow_data[1]==32) & (len(onerow_data)==9):### alone a line-circle
            ...
            for ix in range(quantity):
                x_ix = x_alone + gap * ix * math.cos(line_angle) #這邊的gap 是指中心點的差距
                y_ix = y_alone + gap * ix * math.sin(line_angle)
            
                circle_var.append([radius, s_deg, x_ix, y_ix, parentID, punch_type, onerow_data[0], quantity+1])    #抓取圓形資料
            k = k+3         #將下2行也跳過
        ...
        #######################
            ...
            if (parentID == 0):     ## contour     
                for h in range(len(x_point)):
                    ...
                    dis = math.sqrt((current_x - next_x)**2 + (current_y - next_y)**2)   #distance  # x**2代表x的二次方
                    #print(f'{current_x},{next_x},{current_y},{next_y},{dis}')
                    ## degree
                    if (next_x != current_x):
                        deg = math.atan((next_y - current_y) / (next_x - current_x))    #用math.atan反三角正切取到角度值
                    ...
                    line_var.append([current_x, current_y, dis, deg, parentID, poly_gID,    h+1])
            #if  contour end
                ...
                for k1 in range(4):                         # 取最下面的點再取左邊的點
                    if (y_point[k1] == min(y_point)) & (j==0):
                        a = k1                              #取y值小的點
                        j = 1
                    elif (y_point[k1] == min(y_point)) & (j==1):# 相同小的y值時
                        if x_point[k1]<y_point[a]:          #取x值小的點
                            a = k1
                Octagon_pts = []
                Octagon_pts.append([x_point[a],y_point[a]])     # Octagon_pts[0]
                Octagon_pts.append([x_point[a-7],y_point[a-7]]) # Octagon_pts[1]
                Octagon_pts.append([x_point[a-6],y_point[a-6]]) # Octagon_pts[2]
                Octagon_pts.append([x_point[a-5],y_point[a-5]]) # Octagon_pts[3]
                Octagon_pts.append([x_point[a-4],y_point[a-4]]) # Octagon_pts[4]
                Octagon_pts.append([x_point[a-3],y_point[a-3]]) # Octagon_pts[5]
                Octagon_pts.append([x_point[a-2],y_point[a-2]]) # Octagon_pts[6]
                Octagon_pts.append([x_point[a-1],y_point[a-1]]) # Octagon_pts[7]
        ...
        #######################
            ...
            for com_index in range(int(numEdge)):
                ...
                if len(one_comdata)==2:
                    ...
                    line_subserialnum += 1
                    line_var.append([current_x, current_y, dis, deg, parentID, com_gID, line_subserialnum])
                    ...
                    if (cen_x != current_x):
                        star_deg = math.atan((current_y - cen_y) / (current_x - cen_x))
                    ...
                    arc_subserialnum += 1
                    arc_var.append([cen_x, cen_y, radius, star_deg, working_deg, orientation, parentID, com_gID, arc_subserialnum])
    ...
    if (tool_shape==1): # square
        ...
        for  k2 in range(len(station)):
            if required_dim[0]==0:  ## "沒"有指定刀的尺寸
                ...
                if station[k2,0] == required_dim[0]:
                    a = k2
                    break
...
def tool_expend(tool, tool_num):
    ...
    if tool[tool_num, 2] ==-1:
        ...
        trans_array = np.array([[np.cos(theta), -np.sin(theta)],[np.sin(theta), np.cos(theta)]])
        #旋轉矩陣 
...
def line(line_start_x, line_start_y, distance, line_deg, station, offset_index, line_serial_num, sub_serial_num, overlap):
    ...
    if (station[tool_num,2] == -1):  ######## 圓刀nib ########
        ...
        y_offset = orient * r * math.cos(line_deg) #- (overlap/2) * math.sin(line_deg)
        ...
        while (a>0) :
            ...
            line_x = last_line_xy[0] + one_step * math.cos(line_deg)    #記錄此時加工的點位
        ...
        #旋轉矩陣
        trans_array = np.array([[np.cos(line_deg), -np.sin(line_deg)],[np.sin(line_deg), np.cos(line_deg)]])
...
def circle(r, start_deg, cen_x, cen_y, parentID, tool, punch_type, cir_serial_num):
    
    tool_num = ch_tool(-1, tool, 0, [0,0])
    cir_xy = []
    pitch = 2
    ...
    if (parentID != 0):               #除料範圍為圓內
        ...
        if r == tool[tool_num, 0]:       # 刀具等於洞的大小時
            cir_xy.append([cen_x, cen_y, tool_num, 17, cir_serial_num, 1, ishole]) # punch_type先用17，模擬方便
            ...
            while angle < (2*np.pi):
                ...
                cir_xy.append([x_point, y_point, tool_num, 39, cir_serial_num, 1, ishole])
                angle += feeding_a
...
def arc(cen_x, cen_y, r, s_deg, working_deg, orientation, parentID, station, line_serial_num, sub_serial_num):
    ...
    tool_num = ch_tool(-1, station, 0, [0,0])
    arc_xy = []
    nib_hit_arc = 0
    pitch = 2
    ishole = 0
    ...
    while angle < working_deg:
        ...
        arc_xy.append([x_point, y_point, tool_num, 39, line_serial_num, sub_serial_num, ishole])     ### arc的punch type 只會是39
        nib_hit_arc += 1
        angle += feeding_a
...
def xy_data_rotate(befor_rota_xy, sp2, degree):
    ...
    trans_array = np.array([[np.cos(degree), -np.sin(degree)],[np.sin(degree), np.cos(degree)]])
    #旋轉矩陣
    ...
    for k1 in range(pts_num):  #將每一點都乘上旋轉矩陣，再加上第一個洞的原點
        ...
        af_rota_xy.append([x_abs, y_abs, tool_num, punch_type, rect_serial_num, rect_subserial_num, ishole])
        #將點位記錄在 xy_data 內
...
def rect_hole(L, H, sp1, punch_type, station, hole_roDeg, rect_serial_num, rect_subserial_num):      #做法為：以單一矩形洞做全孔加工，剩下的陣列洞另外處理
    ...
    if punch_type==18: # grid
        tool_num = ch_tool(roDeg, station, 1, [0,0])    #選到適用的刀
        #第三個參數"1"表示要選正方形刀
        ...
        if roDeg==0:    # along 4 sides of a rectangle
            ...
            punch_type = 34
        tool_num = ch_tool(roDeg, station, 0, [0,0])    #選到適用的刀
    ...
    trans_array = np.array([[np.cos(hole_roDeg), -np.sin(hole_roDeg)],[np.sin(hole_roDeg), np.cos(hole_roDeg)]])
    #旋轉矩陣
...
def rect_outside_edge(L, H, sp1, station, roDeg, rect_serial_num):  #做矩形外圍的加工 ## roDeg是弳度制
    ...
    rect_xy.append([x_start, y_start, tool_num, rect_punch_type, rect_serial_num, 1, ishole]) #外邊緣開始工作的第一點
    ...
    for k1 in range(2):
        ...
        while a>0:
            ...
            rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, k1+1, ishole])
        ...
        rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, 2, ishole])
    ...
    rect_xy.append([x_start, y_start, tool_num, rect_punch_type, rect_serial_num, 3, ishole]) #外邊緣開始工作的第一點
    ...
    for k1 in range(2):
        ...
        while b>0:
            ...
            rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, k1+3, ishole])
        ...
        rect_xy.append([x_out, y_out, tool_num, rect_punch_type, rect_serial_num, 4, ishole])
...
def Octagon_hole(Octagon_pts, station, gID, parentID):
    ...
    while True :     
        a = (D2 - L_tool) - (H1-b-w_tool)*slope1 + (H1-b-w_tool)*slope2    # 水平行程
    ...
    distance = math.sqrt((Octagon_pts[7,0]-Octagon_pts[0,0])**2 + (Octagon_pts[7,1]-Octagon_pts[0,1])**2)
    overlap = 0
    ...
    distance = math.sqrt((Octagon_pts[1,0]-Octagon_pts[2,0])**2 + (Octagon_pts[1,1]-Octagon_pts[2,1])**2)
    overlap = 0
    ...
    # line 3
    edge_deg = 135 * math.pi / 180
    distance = math.sqrt((Octagon_pts[3,0]-Octagon_pts[4,0])**2 + (Octagon_pts[3,1]-Octagon_pts[4,1])**2)
    overlap = 0
    offset_index = 1  #由左向"右"加工
    ...
    # line 4
    edge_deg = 45 * math.pi / 180
    distance = math.sqrt((Octagon_pts[6,0]-Octagon_pts[5,0])**2 + (Octagon_pts[6,1]-Octagon_pts[5,1])**2)
    overlap = 0
    offset_index = 0  #由右向"左"加工
...
def feature_calc(xy_data, path):
    ...
    for ix in range(len(xy_data)):
        ...
        if pre_pt[4] != one_xy_data[4]:  ## 同把刀換新線段
            ...
            if one_xy_data[3] == 16:  ## single hit
                single_hit += 1
                time1 += punch_time(0)
                
            elif one_xy_data[3] == 17:  ## along a line hit
                single_hit += 1
                time1 += punch_time(0)
                
            elif one_xy_data[3] == 18:  ## grid
                single_hit += 1
                time1 += punch_time(0)
            ...
            elif one_xy_data[3] == 40:
                nibbling_hit += 1
                time1 += punch_time(1)
                
            elif one_xy_data[3] == 50:
                pass
...
if __name__ == '__main__':
    ...
    path = 'Octagon' #Cir_RectShear_Training_sheet
    sheet_num = 1
    ...
    for sn in range(sheet_num):
        ...
        wholedata, row_data = readfile("Octagon.txt")
        
        line_var, rect_var, circle_var, arc_var, polygon_var = data_to_function(wholedata)                                    
                                                                   
        station = np.zeros([12,3])      #刀具規格[長, 寬, 角度]
        station[0, :] = [20, 4,  0]
        station[1, :] = [20, 4, 90]
        station[2, :] = [10, 2, 30]
        station[3, :] = [10, 2, 45]
        ...
        for k1 in range(len(rect_var)):
            ...
            if parentID==0:     ## rect_outside_edge
                rect_xy = rect_outside_edge(L, H, sp1, station, roDeg, rect_serial_num)
                for j1 in range(len(rect_xy)):
                    j2 = rect_xy[j1]
                    all_rect.append(j2)

            else:       ## hole
                hole_xy_data = rect_hole(L, H, sp1, punch_type, station, roDeg, rect_serial_num, rect_subserial_num)
        ...
        for k3 in range(len(circle_var)):
            ...
            cir_xy = circle(r, start_deg, cen_x, cen_y, parentID, station, punch_type, cir_serial_num)
        ...
        for k1 in polygon_var:
            ...
            hole_xy_data = Octagon_hole(Octagon_pts, station, poly_gID, parentID)
        ...
        xy_data = sorted(xy_data, key = itemgetter(6, 2), reverse = True) # 重新排列，以6、2 的位置排
